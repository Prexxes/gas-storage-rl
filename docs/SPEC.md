...
The held-out CSV path, minimum start date, and rolling-window stride are configured
with `backtest_data_config`, independently of the synthetic price-process
parameters.
...
Validation and test metrics include mean, median, standard deviation, minimum, maximum, interquartile mean raw return, terminal deviation, cumulative cashflow, terminal penalty, and clipped-action counts. Clipped-action metrics report total constrained actions, physical rate/capacity clipping, terminal-feasibility clipping, and the physical-only, terminal-only, and physical-and-terminal intersections.
...
Periodic validation runs after `total_training_env_steps` reaches each configured `eval_freq` interval. After the final policy update, one additional validation row is appended to `evaluations.csv`, even when `total_timesteps` is exactly divisible by `eval_freq`. The best validation model is saved separately from the final model. A risk-adjusted validation model is also saved by maximizing `mean_return_raw - risk_adjusted_std_penalty * std_return_raw`, where `risk_adjusted_std_penalty` defaults to `0.5`. Test and historical backtest evaluation are not part of the training command. They are run manually after model and hyperparameter selection with `gas_storage_rl.evaluation.run_holdout_evaluation`, so holdout results do not influence training-time decisions. Manual holdout evaluation defaults to `best_validation_model.zip`, with `final_model.zip` and `best_risk_adjusted_validation_model.zip` selectable by CLI.

Experiment groups can be evaluated on holdout splits with `gas_storage_rl.evaluation.run_group_holdout_evaluation`. The group runner evaluates each completed or skipped training run with the selected model checkpoint, writes each run's `holdout_<split>_metrics.json` and `final_episode_metrics_<split>.csv`, and writes group-level `holdout_<split>_runs.csv`, `holdout_<split>_episode_metrics.csv`, `perfect_foresight_references_<split>.jsonl`, and `holdout_<split>_summary.json`. Perfect-foresight references are solved once per group because final-seed groups keep the dataset seed fixed. Per-run episode rows contain each policy episode's raw return, terminal outcome, clipping counts, `episode_perfect_foresight_return_raw`, `episode_perfect_foresight_ratio`, and `episode_optimality_gap`. Group summaries aggregate run-level metrics over the seed axis with mean, median, standard deviation, minimum, maximum, interquartile mean, and percentile-bootstrap confidence intervals for the IQM estimator.
